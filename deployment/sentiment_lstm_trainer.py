...
def train():
    ...
    model = tf.keras.Sequential([
        ...
        layers.Bidirectional(layers.LSTM(embedding_dim, activation="tanh", recurrent_activation="sigmoid", recurrent_dropout=0,                     unroll=False, use_bias=True)),
        layers.Dropout(0.5),
        layers.Dense(1)])
    ...
    # Train the model
    epochs = 4
    csv_logger = CSVLogger('training_lstm.log')
    history = model.fit(
        train_ds,
